...
def drawPlanet(planet, stdscr):
    ...
    cx = width // 2
    for i in range(720):
        angle, distance, x, y = Planet_Position(i, planet["radius"], 2*planet["width"], planet["height"], cx, cy, planet["angular_velocity"], planet["initial_position"])
        drawChar(stdscr, int(x), int(y), '.')
    angle, distance, x, y = Planet_Position(config.time, planet["radius"], 2*planet["width"], planet["height"], cx, cy, planet["angular_velocity"], planet["initial_position"])
